...
class Program:
  def __init__(self, prog, args=[], qemu_args=[]): # prog=binary
    # create the logs dir
    try:
    ...
    # プログラムの存在を確認し，引数とハッシュ値を格納 -yuike
    ...
    # 静的解析の初期化 -yuike
    # 基本的にWebUI で使用される static2/static2.py
    # qira_asm の逆アセンブリもこの中で行っている
    # バイナリ解析(独自実装)し，セクション，セグメント，シンボル情報を取得している
    ...
    # bring this back
    # ターゲットバイナリを /tmp/qira_binary にシンボリックリンク -yuike
    ...
    # ターゲットバイナリのフォーマットやアーキテクチャの解析 -yuike
    # 使用する qemu のアーキテクチャを指定
  ...
  def execqira(self, args=[], shouldfork=True):
    ...
    if qira_config.USE_PIN:
      ...
      # qirabinary = qira-[arch_name] = qemu-[arch_name]
      # ユーザモードQEMU を起動させ，バイナリを実行 -yuike
      eargs = [self.qirabinary]+self.defaultargs+args+[self.program]+self.args
    print "*** eargs: ",' '.join(eargs)
    os.execvp(eargs[0], eargs)


# DB の更新をチェックし，トレース -yuike
# qira_webserver.run_server の ポーリング処理から呼ばれる
class Trace:
  ...
  # /tmp/qira_logs/*_strace を読んでいる
  # *_strace をパースし，QEMU がマッピングしたファイル(ライブラリなど)の
  # 情報を表示している
  def read_strace_file(self):
    try:
      f = open(qira_config.TRACE_FILE_BASE+str(int(self.forknum))+"_strace").read()
      print "*** read", qira_config.TRACE_FILE_BASE+str(int(self.forknum))+"_strace"
    except:
      return "no strace"
    ...
    for ff in f.split("\n"):
      ...
      pid = int(ff[1]) # pid
      sc = " ".join(ff[2:]) # system call
      try:
        # システムコールの戻り値の取得
        return_code = int(sc.split(") = ")[1].split(" ")[0], 0)
        fxn = sc.split("(")[0] # system call function
        if fxn == "open" and return_code != -1:
          firststr = sc.split('\"')[1] # open filename (library etc..)
          files[return_code] = firststr
        elif fxn[0:4] == "mmap": # open されたファイルが mmap してた場合
          #print "ff =", ff
          args = sc.split(",")
          sz = int(args[1], 0)
          fil = int(args[4], 0)
          off = int(args[5].split(")")[0], 0)
          mapp = (files[fil], sz, off, return_code) # system call copy?
          if mapp not in self.mapped:
            # if it fails once, don't try again
            self.mapped.append(mapp)
            try:
              # open →  mmapしたファイルを開く処理
              try:
                # QEMU の環境変数を設定していた場合
                f = open(os.environ['QEMU_LD_PREFIX']+"/"+files[fil])
              except:
                # 通常はコッチが呼ばれる
                f = open(files[fil]) # ライブラリファイルなどをオープン
              alldat = f.read() # ライブラリファイルの読込み
              ...
              # 読み込んだライブラリの情報表示
              # sz = サイズ，return_code = マップされた開始アドレス
  ...
  def analysis_thread(self):
    ...
    print "*** started analysis_thread @ analysis_thread: qira_program.py"
    while 1:
      time.sleep(0.2)
      ...
      if self.maxclnum == None or self.db.get_maxclnum() != self.maxclnum:
        ...
        # qira_asm を読み，パース -yuike
        self.program.read_asm_file()
        # 呼び出された命令すべてを self.flow に格納
  ...
  def load_base_memory(self):
    ...
    for ln in f.read().split("\n"):
      # ln には，バイナリと各ライブラリのメモリマップが格納されている
      ln = ln.split(" ")
      if len(ln) < 3:
        continue
      # ss, se には，バイナリやライブラリの先頭/終了アドレスが格納
      # 変換前のアドレス (strace 前)
      (ss, se) = ln[0].split("-")
      ss = int(ss, 16)
      se = int(se, 16)
